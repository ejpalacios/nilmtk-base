    ...
    * Method for measuring ROC curve.
        ...
        for meter in metergroup.meters:
            for chunk in meter.power_series():
                if chunk.empty:
                    ...
                    self.train_on_chunk(
                        how(pd.DataFrame(chunk.resample(self.sample_period))), meter
                    )
    ...
    def train_on_chunk(self, chunk, meter):
        ...
        * Disaggregates only the selected appliance.(TODO: Disaggregates many)
        ...
        if len(edges) > 1:
            ...
            duration = (
                offpower.reset_index().date_time - onpower.reset_index().date_time
            )
            ...
            self.onpower_train = pd.concat([self.onpower_train, onpower]).reset_index(
                drop=True
            )
            self.offpower_train = pd.concat(
                [self.offpower_train, offpower]
            ).reset_index(drop=True)
            self.duration_train = pd.concat(
                [self.duration_train, duration]
            ).reset_index(drop=True)
        ...
        # UPDATE STATS:
        stat_dict = {
            "appliance": meter.identifier[0],
            "instance": meter.identifier[1],
        ...
        if len(self.stats) == 0:
            ...
            for stat in self.stats:
                if (stat["appliance"] == stat_dict["appliance"]) and (
                    ...
                    self.stats[index]["Nevents"] = (
                        self.stats[index]["Nevents"] + number_of_events
                    )
                    instanceFound = True
    ...
    def disaggregate(self, mains, output_datastore):
        ...
        for chunk in mains.power_series():
            ...
            dis_chunk = self.disaggregate_chunk(
                pd.DataFrame(
                    chunk.resample(self.sample_period, how=self.sampling_method)
                )
            )
            ...
            output_datastore.append(
                key=mains_data_location, value=pd.DataFrame(chunk, columns=cols)
            )
    ...
    def disaggregate_chunk(self, chunk):
        ...
        * Disaggregates only the selected appliance.(TODO: Disaggregates many)
            ...
            if not offevents.empty:
                ...
                for offevent in offevents.iterrows():
                    ...
                    likelihood = (
                        ...
                        * self.__pdf(
                            self.duration, (offevent[0] - onevent[0]).total_seconds()
                        )
                    )
        ...
        for row in detections.iterrows():
            ...
            if (
                dis_chunk[
                    (dis_chunk.index >= row[1][0]) & (dis_chunk.index < row[1][1])
                ]
                .sum()
                .values[0]
            ) == 0:
    ...
    def featuresHist(self, **kwargs):
        ...
        bins_onpower = np.arange(
            ...
            (self.onpower_train.max().values[0] - self.onpower_train.min().values[0])
            / 50,
        )
        ...
        bins_offpower = np.arange(
            ...
            (self.offpower_train.max().values[0] - self.offpower_train.min().values[0])
            / 50,
        )
        ...
        bins_duration = np.arange(
            ...
            (self.duration_train.max().values[0] - self.duration_train.min().values[0])
            / 50,
        )
        ...
        x = np.arange(
            bins_onpower.min(),
            bins_onpower.max() + np.diff(bins_onpower)[0],
            np.diff(bins_onpower)[0] / float(1000),
        ).reshape(-1, 1)
        y = self.__pdf(self.onpower, x)
        norm = pd.cut(
            self.onpower_train.onpower, bins=bins_onpower
        ).value_counts().max() / max(y)
        # Plots for Onpower
        ...
        norm = pd.cut(
            self.offpower_train.offpower, bins=bins_offpower
        ).value_counts().max() / max(y)
        # Plots for Offpower
        ...
        norm = pd.cut(
            self.duration_train.duration, bins=bins_duration
        ).value_counts().max() / max(y)
        # Plots for duration
    ...
    def featuresHist_colors(self, **kwargs):
        ...
        for ind in np.arange(len(self.stats)):
            if self.stats[ind]["Nevents"] != 0:
                ...
                ax1.hist(
                    self.onpower_train[start:end].onpower.values,
                    bins=bins_onpower,
                    alpha=0.5,
                )
                ax2.hist(
                    self.offpower_train[start:end].offpower.values,
                    bins=bins_offpower,
                    alpha=0.5,
                )
                ax3.hist(
                    self.duration_train[start:end].duration.values,
                    bins=bins_duration,
                    alpha=0.5,
                )
