...
def check_ac_type(ac_type):
    if ac_type not in AC_TYPES:
        msg = "'{}' is not a valid AC type." " Valid AC types = {}.".format(
            ac_type, AC_TYPES
        )
...
def check_physical_quantity(physical_quantity):
    if physical_quantity not in PHYSICAL_QUANTITIES:
        msg = (
            "'{}' is not a valid physical quantity."
            " Valid physical quantities = {}.".format(
...
def select_best_ac_type(available_ac_types, mains_ac_types=None):
    ...
    if mains_ac_types is None:
        ...
        order_of_preference = [
            ac_type for ac_type in AC_TYPES if ac_type in mains_ac_types
        ]
    ...
    if mains_ac_types is None:
        ...
        warn(
            "None of the AC types recorded by Mains are present in `available_ac_types`."
