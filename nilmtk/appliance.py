...
class Appliance(Hashable):
    ...
    def __init__(self, metadata=None):
        ...
        if (
            ...
            and self.identifier.type not in Appliance.appliance_types
        ):
            warn(
                "'{}' is not a recognised appliance type.".format(self.identifier.type),
                RuntimeWarning,
            )
    ...
    def label(self, pretty=False):
        ...
        if self.identifier.type == "unknown":
            label += ", original name = {}".format(self.metadata.get("original_name"))
        return label
