...
def show_versions():
    ...
    output["System version"] = "{}.{}".format(
        system_information.major, system_information.minor
    )
...
def compute_rmse(ground_truth, predictions, pretty=True):
    ...
    for app_idx, app in enumerate(ground_truth.columns):
        ...
        df_app = pd.DataFrame(
            {"gt": gt_app, "pr": pred_app}, index=gt_app.index
        ).dropna()
    ...
    if pretty:
        for current_label, current_count in app_counts.items():
            ...
            for app_data in rms_error:
                ...
                app = app_data[0]
                app_data[1] = "{} ({}, {})".format(
                    current_label, app.building, app.instance
                )
...
def safe_resample(data, **resample_kwargs):
    ...
    def _resample_chain(data, all_resample_kwargs):
        ...
        if all_resample_kwargs:
            warnings.warn(
                "Not all resample_kwargs were consumed: {}".format(
                    repr(all_resample_kwargs)
                )
            )
