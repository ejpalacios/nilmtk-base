...
class Electric(object):
    ...
    def proportion_of_upstream(self, **load_kwargs):
        ...
        proportion_of_energy = self.total_energy(
    ...
    def average_energy_per_period(
        self, offset_alias="D", use_uptime=True, **load_kwargs
    ):
    ...
    def correlation(self, other, **load_kwargs):
        ...
        numerator = 0.0
        for x_power, y_power in zip(
            self.power_series(**load_kwargs), other.power_series(**load_kwargs)
        ):
    ...
    def plot_power_histogram(
        self, ax=None, load_kwargs=None, plot_kwargs=None, range=None, **hist_kwargs
    ):
        """
        Parameters
        ----------
    ...
    def switch_times(self, threshold=40):
        ...
        for power in self.power_series():
            ...
            datetime_switches.append(
                delta_power_absolute[
                    (delta_power_absolute > threshold)
                ].index.values.tolist()
            )
    ...
    def mutual_information(self, other, k=3, base=2):
        ...
        def avgdigamma(points, dvec):
            ...
            for i in range(N):
                ...
                num_points = len(
                    tree.query_ball_point(points[i], dist - 1e-15, p=float("inf"))
                )
    ...
    def available_power_ac_types(self):
        ...
        """
        warn(
            "`available_power_ac_types` is deprecated.  Please use"
            " `available_ac_types('power')` instead.",
            DeprecationWarning,
        )
    ...
    def activity_histogram(self, period="D", bin_duration="H", **kwargs):
        ...
        if n_bins != int(n_bins):
            raise ValueError(
                "`bin_duration` must exactly divide the" " chosen `period`"
            )
        n_bins = int(n_bins)
    ...
    def plot_activity_histogram(
        self, ax=None, period="D", bin_duration="H", plot_kwargs=None, **kwargs
    ):
        # Some common labels
        freq_labels = {
            "H": "hour",
        ...
        if ax is None:
            ax = plt.gca()
        hist = self.activity_histogram(
            bin_duration=bin_duration, period=period, **kwargs
        )
        if plot_kwargs is None:
            plot_kwargs = {}
        n_bins = len(hist)
...
def get_activations(
    chunk, min_off_duration=0, min_on_duration=0, border=1, on_power_threshold=5
):
    """Returns runs of an appliance.
    ...
    if min_off_duration > 0:
        off_durations = (
            chunk.index[switch_on_events[1:]].values
