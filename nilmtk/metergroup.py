...
class MeterGroup(Electric):
    ...
    def import_metadata(self, store, elec_meters, appliances, building_id):
        ...
        if not elec_meters:
            warnings.warn(
                "Building {} has an empty 'elec_meters' object.".format(
                    building_id.instance
                ),
                RuntimeWarning,
            )
        if not appliances:
            warnings.warn(
                "Building {} has an empty 'appliances' list.".format(
                    building_id.instance
                ),
                RuntimeWarning,
            )
        ...
        meters_to_disable = [
            ...
            if isinstance(m, ElecMeter) and m.metadata.get("disabled")
        ]
    ...
    def dominant_appliance(self):
        dominant_appliances = [meter.dominant_appliance() for meter in self.meters]
        ...
        * `{'dataset': 'redd', 'building': 3, 'type': 'toaster', 'instance': 2}`
        ...
        if isinstance(key, str):
            ...
            if isinstance(key.instance, tuple):
                ...
                for group in self.nested_metergroups():
                    if (
                        set(group.instance()) == set(key.instance)
                        and group.building() == key.building
                        and group.dataset() == key.dataset
                    ):
                        return group
        ...
        # find MeterGroup from list of ElecMeterIDs
            ...
            if len(meters) == 1:
                ...
                raise Exception("search terms match {} appliances".format(len(meters)))
            else:
                raise KeyError(key)
            ...
            if n_meters_found > 1:
                raise Exception(
                    "{} meters found with instance == {}: {}".format(
                        n_meters_found, key, meters_found
                    )
                )
    ...
    def values_for_appliance_metadata_key(
        self, key, only_consider_dominant_appliance=True
    ):
        """
        Parameters
        ----------
    ...
    def _convert_physical_quantity_and_ac_type_to_cols(self, **kwargs):
        ...
        for meter in self.meters:
            ...
            new_kwargs = meter._convert_physical_quantity_and_ac_type_to_cols(
                **kwargs_copy
            )
    ...
    def mains(self):
        ...
        if self.contains_meters_from_multiple_buildings():
            msg = (
                "This MeterGroup contains meters from buildings '{}'."
                " It only makes sense to get `mains` if the MeterGroup"
                " contains meters from a single building.".format(self.building())
            )
    ...
    def upstream_meter(self):
        ...
        if len(unique_upstream_meters) > 1:
            raise RuntimeError(
                "{:d} upstream meters found for meter group."
    ...
    def _collect_stats_on_all_meters(self, load_kwargs, func, full_results):
        ...
        for meter in self.meters:
            ...
            if (
                ...
                and not meter.store.all_sections_smaller_than_chunksize
            ):
                warnings.warn(
                    "at least one section requested from '{}' required"
    ...
    def _check_kwargs_for_full_results_and_sections(self, load_kwargs):
        if (
            load_kwargs.get("full_results")
            and "sections" not in load_kwargs
            and len(self.meters) > 1
        ):
            raise RuntimeError(
                "MeterGroup stats can only return full results"
    ...
    def good_sections(self, **kwargs):
        ...
        if self.meters:
            if len(self.meters) > 1:
                warnings.warn(
                    "As a quick implementation we only get Good Sections from"
    ...
    def available_ac_types(self, physical_quantity):
        ...
        """
        all_ac_types = [
            meter.available_ac_types(physical_quantity) for meter in self.meters
        ]
    ...
    def available_physical_quantities(self):
        ...
        """
        all_physical_quants = [
            meter.available_physical_quantities() for meter in self.meters
        ]
        return list(set(flatten_2d_list(all_physical_quants)))

    def energy_per_meter(
        self, per_period=None, mains=None, use_meter_labels=False, **load_kwargs
    ):
    ...
    def _energy_per_meter_with_remainder(
        self, energy_per_meter, mains, per_period, **kwargs
    ):
    ...
    def train_test_split(self, train_fraction=0.5):
        ...
        appx_num_records_in_each_good_section = [
            int((ts.end - ts.start).total_seconds() / sample_period)
            for ts in good_sections
        ]
        ...
        if len(good_sections) == 1:
            ...
            while records_remaining:
                for i, records_in_section in enumerate(
                    appx_num_records_in_each_good_section
                ):
                    if records_remaining > records_in_section:
                        ...
                        split_point = good_sections[i].start + timedelta(
                            seconds=sample_period * records_remaining
                        )
                        return split_point
    ...
    def select_top_k(
        self, k=5, by="energy", asc=False, group_remainder=False, **kwargs
    ):
        ...
        **kwargs : key word arguments to pass to load()
        ...
        if (
            top_k_elec_meter_ids
            and type(top_k_elec_meter_ids[0]) is tuple
            and len(top_k_elec_meter_ids[0]) == 3
        ):
        ...
        if group_remainder:
            ...
            if (
                remainder_ids
                and type(remainder_ids[0]) is tuple
                and len(remainder_ids[0]) == 3
            ):
    ...
    def _plot_sankey(self, **load_kwargs):
        ...
        # Plot
        Sankey(flows=energy_ratio, labels=labels, orientations=orientations).finish()
    ...
    def _plot_energy_bar(self, ax=None, mains=None, **load_kwargs):
        ...
        ax = pd.DataFrame(energy).T.plot(
            kind="bar", stacked=True, grid=True, edgecolor="none", legend=False, width=2
        )
        ax.set_xticks([])
        ax.set_ylabel(
    ...
    def describe(self, compute_expensive_stats=True, **kwargs):
        ...
        if compute_expensive_stats:
            ...
            series["dropout_rates_ignoring_gaps"] = "min={}, mean={}, max={}".format(
                dropout_rates.min(), dropout_rates.mean(), dropout_rates.max()
            )
...
def combine_chunks_from_generators(index, columns, meters, kwargs):
    ...
    for meter in meters:
        ...
        physical_quantities = chunk_from_next_meter.columns.get_level_values(
            "physical_quantity"
        )
        columns_to_average = set(PHYSICAL_QUANTITIES_TO_AVERAGE).intersection(
            physical_quantities
        )
