...
class ElecMeter(Hashable, Electric):
    ...
    def _check_store(self):
        if self.store is None:
            raise RuntimeError(
                "ElecMeter needs `store` attribute set to an"
    ...
    def upstream_meter(self, raise_warning=True):
        ...
        if (
            upstream_meter_in_building is not None
            and upstream_meter_in_building != self.identifier.building
        ):
    ...
    def available_ac_types(self, physical_quantity):
        ...
        return [
            ...
            if m["physical_quantity"] == physical_quantity and "type" in m
        ]
    ...
    def available_columns(self):
        ...
        measurements = self.device["measurements"]
        return list(
            set([(m["physical_quantity"], m.get("type", "")) for m in measurements])
        )
        ...
        **kwargs : any other key word arguments to pass to `self.store.load()`
        ...
        if kwargs.get("resample"):
            ...
            if "limit" not in resample_kwargs:
                ...
                if (
                    default_sample_period is not None
                    and sample_period < default_sample_period
                ):
                    warn(
    ...
    def _physical_quantity_to_columns(self, physical_quantity):
        ...
        if isinstance(physical_quantity, list):
            cols2d = [
                self._physical_quantity_to_columns(p_q) for p_q in physical_quantity
            ]
            return list(set(flatten_2d_list(cols2d)))

        check_physical_quantity(physical_quantity)
        cols_matching = [
            col for col in self.available_columns() if col[0] == physical_quantity
        ]
        return cols_matching
    ...
    def _convert_physical_quantity_and_ac_type_to_cols(
        ...
        if columns:
            if ac_type or physical_quantity:
                raise ValueError(
                    "Cannot use `ac_type` and/or `physical_quantity`"
        ...
        if not (ac_type or physical_quantity):
            ...
            if physical_quantity:
                ...
                if not cols_matching_pq:
                    msg += "Physical quantity '{}' not available. ".format(
                        physical_quantity
                    )
                if columns:
                    columns = list(set(columns).intersection(cols_matching_pq))
                    if not columns:
                        msg += "No measurement matching ({}, {}). ".format(
                            physical_quantity, ac_type
                        )
                else:
    ...
    def key_for_cached_stat(self, stat_name):
        ...
        return "building{:d}/elec/cache/meter{}/{:s}".format(
