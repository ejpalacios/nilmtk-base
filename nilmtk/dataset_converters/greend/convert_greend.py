...
def _get_blocks(filename):
    ...
    special_check = (
        "dataset_2014-01-28.csv" in filename and "building5" in filename
    ) or ("dataset_2014-09-02.csv" in filename and "building6" in filename)
    ...
    with open(filename, "r") as f:
        for line in f:
            ...
            if special_check:
                if (
                    "0.072.172091508705606" in line
                    or "1409660828.0753369,NULL,NUL" == line
                ):
                    continue
...
def convert_greend(greend_path, hdf_filename, use_mp=True):
    ...
    metadata_dir = join(
        get_module_directory(), "dataset_converters", "greend", "metadata"
    )
