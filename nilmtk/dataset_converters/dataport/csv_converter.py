...
def create_nilmtk_metadata(building_id, dataport_metadata):
    ...
    locality = ", ".join(
        ...
        + dataport_metadata.loc[
            dataport_metadata.dataid == building_id, "state"
        ].tolist()
    )
    building_metadata = {
...
def extract_building_data(csv_filename, b_id, b_metadata, chunksize, csv_b_cache):
    ...
    csv_data_gen = pd.read_csv(
        csv_filename, engine="c", chunksize=chunksize, index_col=["localminute"]
    )
    b_isnew = True
    b_data = pd.DataFrame()
    ...
    for i, csv_chunk in enumerate(csv_data_gen):
        ...
        if b_id in b_list:
            ...
            b_chunk.index = pd.to_datetime(
                b_chunk.index, utc=True, infer_datetime_format=True
            )
...
def convert_dataport(csv_filenames, metadata_path, hdf_filename, chunksize=1e6):
    ...
    csv_building_cache = (
        {}
    )  # cache of the building list per file to speed up the search
    ...
    for b_id in dataids:
        ...
        for csv_file in csv_filenames:
            ...
            if (
                csv_file not in csv_building_cache
                or b_id in csv_building_cache[csv_file]
            ):
