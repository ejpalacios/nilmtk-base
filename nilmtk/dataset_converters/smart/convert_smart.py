...
def convert_smart(smart_path, output_filename, format="HDF"):
    ...
    save_yaml_to_datastore(
        join(get_module_directory(), "dataset_converters", "smart", "metadata"), store
    )
    store.close()
    ...
    for house_id in houses:
        ...
        for y in years:
            ...
            if not meters_paths_csv:
                ...
                df_year = reduce(
                    lambda left, right: left.join(right, lsuffix="_1", rsuffix="_2"),
                    data_frames,
                )
...
def _find_year(input_path, house_id):
    ...
    year_names = [
        p
        for p in listdir(input_path + "/" + "Home" + house_id)
        if isdir(join(input_path + "/" + "Home" + house_id, p))
    ]
    return year_names
...
def _find_all_csv_paths(input_path, house_id, year):
    ...
    for txt_path in txt_paths:
        ...
        if "30" in lines[-2]:
            ...
            paths.append(house_year_path + "/" + name_temp + "_" + year + ".csv")
