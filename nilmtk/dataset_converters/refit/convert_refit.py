...
def convert_refit(input_path, output_filename, format="HDF"):
    ...
    save_yaml_to_datastore(
        join(get_module_directory(), "dataset_converters", "refit", "metadata"), store
    )
    store.close()
...
def _convert(input_path, store, tz, sort_index=True):
    ...
    for house_id in houses:
        ...
        stdout.flush()
        csv_filename = join(
            input_path, prefix + "House" + suffix + str(house_id) + ".csv"
        )
        ...
        if not exists(csv_filename):
            raise RuntimeError(
                "Could not find REFIT files. Please check the provided folder."
            )
