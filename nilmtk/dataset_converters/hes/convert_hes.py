...
FILENAMES = [
    "agd-{s}/appliance_group_data-{s}.csv".format(s=s)
    for s in ["1a", "1b", "1c", "1d", "2", "3"]
]
...
def convert_hes(data_dir, output_filename, format="HDF", max_chunks=None):
    ...
    original_metadata_dir = join(
        get_module_directory(), "dataset_converters", "hes", "metadata"
    )
    ...
    for filename in FILENAMES:
        ...
        try:
            reader = pd.read_csv(
                full_filename, names=COL_NAMES, index_col=False, chunksize=CHUNKSIZE
            )
        except IOError as e:
            print(e, file=stderr)
            continue
    ...
    for hes_house_id in house_codes:
        ...
        for appliance_code in house_appliance_codes[hes_house_id]:
            ...
            lookup_row = hes_to_nilmtk_appliance_lookup[
                hes_to_nilmtk_appliance_lookup.Code == appliance_code
            ].iloc[0]
            appliance_metadata = {
