...
class Results(object):
    ...
    def append(self, timeframe, new_results):
        ...
        if not isinstance(timeframe, TimeFrame):
            raise TypeError(
                "`timeframe` must be of type 'nilmtk.TimeFrame',"
                " not '{}' type.".format(type(timeframe))
            )
        if not isinstance(new_results, dict):
            raise TypeError(
                "`new_results` must of a dict, not '{}' type.".format(type(new_results))
            )
    ...
    def unify(self, other):
        ...
        for i, row in self._data.iterrows():
            if other._data["end"].loc[i] != row["end"] or i not in other._data.index:
                raise RuntimeError(
                    "The sections we are trying to merge"
    ...
    def timeframes(self):
        ...
        # over the rows.
        return [
            TimeFrame(self._data.index[i], self._data.iloc[i]["end"])
